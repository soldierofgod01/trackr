...
export function ScannerModule() {
  ...
  const tokensWithLivePrices = useMemo(() => {
    return sourceTokens.map((t) => {
      ...
      // Live stream updates the DISPLAYED PRICE only. priceChange24h stays
      // from the API (CoinGecko) so gainers/losers filters use consistent
      // calendar-day numbers — mixing in the WS rolling-window % broke them.
      if (live) {
        return { ...t, priceUsd: live.price };
      }
      return t;
    });
