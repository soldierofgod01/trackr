...
export function ScannerModule() {
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      let cmp = 0;
      switch (sort) {
        case "volume":  cmp = b.volume24hUsd - a.volume24hUsd; break;
        case "change":  cmp = b.priceChange24hPct - a.priceChange24hPct; break;
        case "oi":      cmp = b.openInterestUsd - a.openInterestUsd; break;
        case "funding": cmp = b.fundingRate8h - a.fundingRate8h; break;
        default:        cmp = 0;
      }
      return cmp * dir;
    });
    return list;
  }, [withLive, screen, sort, search, sortDir]);
    ...
    <div className="flex-1 overflow-y-auto bg-bg-base">
      <div className="px-8 md:px-12 py-9 max-w-[1500px] mx-auto">
        ...
        <div className="ds-panel mb-4 px-4 py-3 flex items-center gap-4 flex-wrap">
          ...
          {/* Right cluster: sort + search */}
          <div className="ml-auto flex items-center gap-3 flex-wrap">
            {/* Sort — click active button again to flip direction */}
              ...
              {SORTS.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 h-7 rounded-[6px] text-[12.5px] font-medium transition-colors flex items-center gap-1 ${
                      active
                        ? "bg-elevated text-txt-primary"
        ...
        {/* ─── Table ─── */}
        <div className="ds-panel ds-panel-signature overflow-hidden">
          {/* Column header */}
          <div
...
function StatTile({
  ...
  return (
    <div className="ds-panel ds-stat-accent px-4 py-3">
...
function MarketRow({
  ...
  return (
    <div
      onClick={onClick}
      className="grid gap-4 px-5 h-[52px] items-center border-b border-border last:border-0 ds-row-hover cursor-pointer"
      style={{
        gridTemplateColumns:
          "minmax(150px,1.4fr) 130px 100px 130px 110px 120px 110px",
      }}
